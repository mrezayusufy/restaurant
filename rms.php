...
class rms{
	...
	{
		$this->query = "SELECT * FROM visitor_table 
	...
	{
		$this->query = "SELECT MAX(order_number) AS order_number FROM order_table";
	...
	{
		$this->query = "SELECT * FROM user_table 
	...
	{
		$this->query = "SELECT restaurant_timezone FROM restaurant_table 
		LIMIT 1
		";
		$result = $this->get_result();
		foreach($result as $row)
